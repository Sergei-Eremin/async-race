...
export class API {
    ...
    async switchEngineState(id: string, status: 'started' | 'stopped' | 'drive') {
        try {
            try {
                ...
                if (data.status === 429) throw new Error('заебал спамить');
                if (data.status === 200) return data.json();
            } catch (error) {
                if (error instanceof Error) {
                    console.log(error, 'ошибка нижнего кетча');
                }
            }
        } catch (error) {
            if (error instanceof Error) {
                console.log(error, 'ошибка верхнего кетча');
            }
        }
    }
    ...
    async getWinners(page: number = 1, limit: number = 10, sort: string = 'id', order: 'ASC' | 'DESC' = 'ASC') {
        const data = await fetch(
            `${this.BASE_URL}/winners?_page=${page}&_limit=${limit}&_sort=${sort}&_order=${order}`
        );
        return data.json();
    }
