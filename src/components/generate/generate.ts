...
export class Generate {
    ...
    createInputListeners(event: Event, carTrack: CarTrack, api: API, app: App, createInput: Element) {
        const target = event?.target as HTMLElement;
        if (target.tagName === 'BUTTON') {
    ...
    async generateWinners(
        ...
        tableBody.innerHTML = '';
        setTimeout(
            () => arrOfWinnersWithCars.forEach((elem, i) => tableBody?.append(winners.createTr(elem, i, app))),
            250
        );
    }
    ...
    inputsFiller(app: App, api: API, carTrack: CarTrack) {
